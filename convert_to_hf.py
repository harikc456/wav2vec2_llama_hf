...
def _map_fairseq2_to_ctc_hf_keys(fairseq2_state_dict: dict) -> dict:
    """
    Map fairseq2 state dict keys to OmnilingualModelForCTC format.

    This function handles the key naming differences between fairseq2 and HF CTC.
    """
    mapped_dict = {}
    ...
    for old_key, value in fairseq2_state_dict.items():
        ...
        # Map standalone encoder keys (if not under encoder_frontend)
            ...
            #     new_key = old_key.replace("encoder.layer_norm", "wav2vec2.encoder.layer_norm")

            elif "encoder.layer_norm" in temp_key:
                new_key = old_key.replace("encoder.layer_norm", "encoder_layer_norm")

            elif 'pos_encoder.conv.' in temp_key:
                new_key = temp_key.replace('pos_encoder.conv.', 'wav2vec2.encoder.pos_conv_embed.conv.')
        ...
        # Map the final projection layer for CTC (without Llama decoder)
        elif old_key == 'final_proj.weight':
            new_key = 'lm_head.weight'

        elif old_key == 'final_proj.bias':
...
def _map_fairseq2_to_hf_keys(fairseq2_state_dict: dict, config) -> dict:
    """
    Map fairseq2 state dict keys to HuggingFace format for LLM variant.

    This function handles the key naming differences between fairseq2 and HF for LLM variant.
    """
    mapped_dict = {}
...
def load_from_fairseq2_checkpoint_ctc(
    fairseq2_checkpoint_path: str,
    config: Wav2Vec2Config = None,
    output_path: str = None,
    auto_detect_langs: bool = True,
    debug: bool = False
) -> OmnilingualModelForCTC:
    """
    Load a pretrained CTC model from fairseq2 checkpoint.

    Args:
        fairseq2_checkpoint_path: Path to fairseq2 .pt checkpoint file
        config: Wav2Vec2Config for CTC model
        output_path: Path to save the converted Hugging Face model
    ...
    Returns:
        Loaded OmnilingualModelForCTC model

    Example:
        >>> model = load_from_fairseq2_checkpoint_ctc("path/to/checkpoint.pt", config, output_path)
    """
    ...
    # 1. Detect vocab size from final_proj (for CTC) - this is the TRUE vocab size
    if 'final_proj.weight' in state_dict:
        detected_vocab_size = state_dict['final_proj.weight'].shape[0]
        print(f"  ✓ Vocab size (from final_proj): {detected_vocab_size}")
        config.vocab_size = detected_vocab_size

    # 2. Detect encoder hidden size
    if 'encoder.layer_norm.weight' in state_dict or 'encoder_frontend.layer_norm.weight' in state_dict:
        layer_norm_key = 'encoder.layer_norm.weight' if 'encoder.layer_norm.weight' in state_dict else 'encoder_frontend.layer_norm.weight'
        detected_encoder_dim = state_dict[layer_norm_key].shape[0]
        print(f"  ✓ Encoder hidden size: {detected_encoder_dim}")
        config.hidden_size = detected_encoder_dim

    print(f"\nCreating CTC model with detected config...")
    print(f"  - Vocab size: {config.vocab_size}")
    print(f"  - Hidden size: {config.hidden_size}")

    # Create model with corrected config
    model = OmnilingualModelForCTC(config).to(device)

    # Map fairseq2 state dict keys to HuggingFace format
    print(f"\nMapping state dict keys for CTC model...")
    print(f"\nCreating CTC model on {device}...")
    mapped_state_dict = _map_fairseq2_to_ctc_hf_keys(state_dict)

    # Load weights
    print(f"\nLoading weights into CTC model...")
    ...
    print(f"Saving converted CTC model to: {output_path}")
    model.save_pretrained(output_path)
    print("CTC Model saved successfully.")
    return model
...
def load_from_fairseq2_checkpoint(
    ...
    """
    Load a pretrained model from fairseq2 checkpoint for LLM variant.
    ...
    mapped_state_dict = _map_fairseq2_to_hf_keys(state_dict, config)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_variant', type=str, choices=['ctc', 'llm'], default='llm',
                        help='Model variant: ctc (without Llama decoder) or llm (with Llama decoder)')

    args = parser.parse_args()

    if args.model_variant == 'ctc':
        # For CTC model, we only need Wav2Vec2 config
        wav2vec2_config = Wav2Vec2Config(
            ...
            # ASR head
            vocab_size=10288,  # Will be overwritten by detection
            final_dropout=0.0,
        ...
        print(f"Loading CTC model from: {args.fairseq_checkpoint_path}")
        load_from_fairseq2_checkpoint_ctc(args.fairseq_checkpoint_path, wav2vec2_config, args.output_path)
    else:
        # For LLM model, we need both configs
        vocab_size = 10288
        ...
        print(f"Loading LLM model from: {args.fairseq_checkpoint_path}")
